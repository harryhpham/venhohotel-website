...
function RoomCard({ room, lang, fromPrice, perNight, contactPrice, viewLakeTag }: RoomCardProps) {
  ...
  return (
    <Link href={`/phong/${room.slug}`} className="group block">
      <div className="aspect-[4/3] relative overflow-hidden mb-5">
        ...
        {room.view && (
          <div className="absolute top-3 left-3 z-10">
            <span className="label-tag bg-[#C9A84C] text-white px-2 py-1">{viewLakeTag}</span>
          </div>
        )}
        {images.length > 1 && (
      ...
      <h3 className="font-display text-xl md:text-2xl text-[#1A1A1A] group-hover:text-[#C9A84C] transition-colors mb-2">{name}</h3>
      <p className="font-sans text-[#6B6B6B] text-sm mb-3 line-clamp-2">{description}</p>
      {room.priceVnd ? (
        <p className="font-mono text-[#1A1A1A] text-sm">
          {fromPrice} <span className="text-[#C9A84C] font-medium">
            {lang === "en" ? `$${room.priceUsd}` : `${room.priceVnd.toLocaleString("vi-VN")}đ`}
          </span>{perNight}
        </p>
      ) : (
        <p className="font-sans text-[#6B6B6B] text-sm italic">{contactPrice}</p>
      )}
    </Link>
  );
}

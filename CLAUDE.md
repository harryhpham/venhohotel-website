...
| **Module 10 — Operating Center + Creative Studio** | `projects/03_AI_STUDIO/venho-ai-studio/dashboard/` + `ui/studio_app.py` | **COMPLETE v2.0 + Creative Studio (2026-07-09)** · 7 tests · Full suite 430/430 · Plan: `VENHO_AI_STUDIO_Module_10_Dashboard_Plan_v1_2.md` · Founder-first Streamlit UI: Operating System + Mode A + Mode B + **3 Creative Studio modes**: Tạo Ảnh AI · Tạo Social Post · Tạo Video Script · `Path(__file__).resolve()` fix cho Streamlit relative `__file__` · timeout 300s cho gpt-image-2 · action-first prompt (action dẫn đầu, default pose bị strip khi có action) · `use_ref` toggle: bật = portrait/standing (score 9) · tắt = full-body action như đạp xe/chạy (score 7–8.5) |

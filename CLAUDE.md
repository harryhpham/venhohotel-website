...
| **Module 03 — Validator Studio** | `projects/03_AI_STUDIO/venho-ai-studio/validator_studio/` | **ALL COMPLETE (2026-07-08)** · 26 tests · 0 API call · Plan: `VENHO_AI_STUDIO_Module_03_Validator_Studio_Plan_v1_1.md` · 4 validator types: image/prompt/face/content · Scoring: AI observe enum → code score deterministic · Kill-switch: forbidden severity=high → cap=40, verdict=regenerate · Face: 07F binary gates + weighted score, grounding OFF · Content: brand_fit/tone/clarity/CTA/language_fit · CLI: `venho validate image\|prompt\|face\|content` · **FORBIDDEN = policy `curated` (2026-08-07)** — validator chỉ gửi rule `curated` sang AI, fallback `observed` khi subject không có overlay; rule vi phạm = severity high = kill-switch = tốn thêm 1 ảnh · **Face gate không xét tóc/biểu cảm** — DNA duyệt nhiều kiểu tóc + nhiều biểu cảm nên chúng là VARIABLE, không phải tín hiệu nhận dạng (`linh-an-master-face.png` từng bị chính gate của nó reject 0.0 vì để tóc xoã → 88.26) |
...
| **Creative Studio (tạo ảnh AI trong VenHo OS)** | Repo riêng `venho-os/` — `src/lib/studio/`, `src/app/api/v1/studio/generate-image/` | **Audit + fix + chống tốn phí (2026-08-07)** · Chi tiết đầy đủ: `venho-os/CHANGELOG.md` + `venho-os/task_memory.md` · **Text người dùng gõ thắng dropdown Scenario** (`scene-intent.ts`) — trước đây prompt-builder âm thầm viết lại "trên rooftop" thành "trên vỉa hè" theo dropdown mặc định · **Biểu cảm tách khỏi identity lock** · **3 cổng validate**: image (DNA) + face (07F) + **intent** (`venho-social-content-agent/validate_intent.py` — ảnh có đúng thứ người dùng gõ không) · **Retry tối đa 1 lần và chỉ khi ảnh SAI** (kill-switch hoặc intent trượt) — KHÔNG dùng `!approved` vì ngưỡng approve là 90 còn ảnh tốt thật rơi 82–88, dùng `!approved` sẽ retry gần như mọi lần chạy và nhân đôi phí gpt-image-2 · Dải `usable` 80 · **Pre-flight chặn trước khi gọi gpt-image-2** (`preflight.ts`, 0 chi phí, chạy cả UI lẫn route) · **`POST /api/v1/studio/quality-rating`** lưu đánh giá Tốt/Tạm/Kém của Harry vào manifest để đối chiếu ngưỡng 90 với mắt thật |
